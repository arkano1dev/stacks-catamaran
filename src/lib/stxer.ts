...
import { asciiToBytes, hexToBytes, intToBytes } from '@stacks/common';
...
export async function runSimulation(
  ...
  const body = new Uint8Array([
    // ...asciiToBytes('sim-v1'),
    // ...intToBytes(block_height, 8),
    // ...(block_hash.startsWith('0x') ? hexToBytes(block_hash.substring(2)) : hexToBytes(block_hash)),
    // ...txs
    //   .map(t => {
...
--------------------------------`
    ...
    const nextNonce = async (sender: string) => {
      ...
      if (nonce == null) {
        ...
        const account: AccountDataResponse = await richFetch(url).then(r => r.json());
